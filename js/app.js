...
function pushHistory(key, label){
  ...
  if(el){
    el.innerHTML = histories[key].length
      ? '최근 기록 · ' + histories[key].map(h=>`<b>${h}</b>`).join(' · ')
      : '';
  }
}
...
function renderEnchant(){
  ...
  const chain = `${e.base} → ${e.t1}` + (chosenT2 ? ` → ${chosenT2}` : ' (2차 강화 없음/미확인)');
...
function renderCombo(){
  ...
  const notes = picked.map(p=>`<div class="result-note">· <b style="color:var(--parchment)">${p[0]}</b> — ${p[1]}</div>`).join('');
...
document.addEventListener('DOMContentLoaded', async()=>{await loadGameData();});
